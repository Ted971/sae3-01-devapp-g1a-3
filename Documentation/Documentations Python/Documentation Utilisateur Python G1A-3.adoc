...
link:file:///home/username[Documentation d'installation]
...
* Le programme boucle à l'infini afin d'assurer la gestion de la fréquence d'écriture dans les fichiers capteurs. Pour ce faire, on crée une alarme _signal.alarm_ qui s'active toute les _x_ minutes selon la fréquence d'écriture paramétrée dans le fichier de configuration. Le programme dors ensuite en attendant que l'alarme soit déclenchée.
* Après que l'intervalle de temps soit passé, on remet a zéro le dictionnaire _StockDico_)_ et la liste _StockSeuil_ afin que chaque valeur écrite dans les fichiers capteurs soit unique.
=====
====
=================